...
func (cli *DockerCli) CmdBuild(args ...string) error {
	...
	bind := cmd.Bool([]string{"b", "-bind"}, false, "Bind the context instead of uploading it")
	if err := cmd.Parse(args); err != nil {
		return nil
	}
	if cmd.NArg() != 1 {
	...
	if cmd.Arg(0) == "-" {
		...
		if *bind {
			if contextPath, err = filepath.Abs(root) ; err != nil {
				// should never happen since we already called os.Stat
				return err;
			}
		} else {
